...
namespace Fungus 
{
    ...
    {   
        ...
        private PlayerPointClick player;
        private NavMeshAgent agent;

        void Awake () 
        {
            player = GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerPointClick>();
        ...
        {
            while(Vector3.Distance(clickableObject.transform.position, player.transform.position) > clickableObject.activateDistance)
            {
                yield return new WaitForSeconds(0.1f);
            }
           
            if((Vector3.Distance(clickableObject.transform.position, player.transform.position) <= clickableObject.activateDistance))
            {
